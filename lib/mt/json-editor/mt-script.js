...
class TagBoxEditor extends JSONEditor.AbstractEditor {
	...
	build() {
		...
		this.input.placeholder = 'Nhập tag và nhấn Enter';
	...
	setValue(value) {
		if (Array.isArray(value)) {
			this.tags = value;
			this.renderTags();
			// this.delegate.updateValue(this.tags);
		}
		else if (typeof value == 'string') {
			let pathValue = value.split(',');
			for (let v of pathValue)
				v = v.trim();
			this.tags = pathValue;
			this.renderTags();
		}
...
export default { RateRegister, TagBoxRegister };