...
let mtDiagram = {
	...
	async init() {
		...
		// Import Library
		await mt.lib.import(['mermaid','CodeMirror','svg-pan-zoom']);
		...
		// Init UI
		let htmlTypeOpts = '';
		for (let typeCode in this.h_type) {
			let type = this.h_type[typeCode];

			// process exam
			type.exam = trimSource(type.exam);

			// Collect option
			htmlTypeOpts += `<option value="${typeCode}">${type.name}</option>`;
		}
		this.e_type.innerHTML = htmlTypeOpts;
	},
	async btnRender() { // Render Mermaid
		try {

			// Prepare
			let code = this.m_editor.getValue();

			// Lib Call
			const uniqueId = 'mermaid-diagram-' + Date.now();
			let result = await mermaid.render(uniqueId, code); // Mermaid render
			...
			// Log
			mt.h_debug && console.log('[mt.diagram] btnRender', { code, uniqueId, result });
		}
		catch (ex) {
	...
	onChangeType() {
		...
		// Prepare
		let typeCode = this.e_type.value;
		let type = this.h_type[typeCode];

		// Tự động điền ví dụ
		this.m_editor.setValue(type.exam);

		this.btnRender();
	},
}
export default mtDiagram;
