var mt = {
	dev: function() {
		...
		mt.url.setType("GET");
		mt.url.set('https://beta.nhaccuatui.com/api/');
...
// /api/users?delay=3
	...
	url: {
			...
			xhr.onreadystatechange = () => {
				if (xhr.readyState == 4) {
					if (xhr.status == 200) {
						...
						let p = mt.url.utl_htmlErr("[404] Page not found");
						mt.response.set(p);
					} else if (xhr.status == 401) {
						let p = mt.url.utl_htmlErr("[401] Authorization required");
						mt.response.set(p);
					}
				}
			};
		...
		request2: function() {
			...
			// Get data
			let typeRequest = mt.url.getType().toLowerCase();
			let fullURL = mt.url.get() + mt.params.get();

			// CALL API
			mt.effect.loading(true);
			axios[typeRequest](fullURL)
				.then(response => {
					if (response.status == 200) {
						let responseType = response.headers['content-type']
						if (responseType.includes('application/json')) {
							mt.response.set(response.data, 'json');
						} else if (responseType.includes('text/html')) {
							mt.response.set(response.data, 'html');
						} else {
							console.log(responseType);
						}
					} else {
